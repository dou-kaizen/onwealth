 ...
 *   3. Update the @Global() table in the architecture docs
 */
...
async function findModuleFiles(): Promise<string[]> {
  const files: string[] = []
  for await (const file of glob('src/**/*.module.ts', { cwd: PKG_ROOT })) {
    files.push(file)
  }
  return files
}
...
describe('architecture: @Global() usage', () => {
  ...
  it('every @Global() module must have @global-approved comment on the same line', () => {
    ...
    for (const file of moduleFiles) {
      const content = fs.readFileSync(path.join(PKG_ROOT, file), 'utf8')
